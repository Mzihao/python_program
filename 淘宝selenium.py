...
def get_block():
    ...
    doc = etree.HTML(html)
    failure = doc.xpath("//div[@class='errloading']/span/text()")
    if failure:
        browser.get("https://www.taobao.com/")
        input = wait.until(
            EC.presence_of_element_located((By.CSS_SELECTOR, '#ks-component163 div.search-combobox-input-wrap > input')))
        submit = wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, '.search-wrap div.search-button > button.btn-search.tb-bg')))
        input.clear()
