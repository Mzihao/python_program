...
html=request.urlopen(req).read().decode("utf-8")    #读取网页源代码内容
wzgz='<li><a href="(.*?)" class="provider-item"><div class="col-media"><div class="cm-left avatar"><div class="avatar"><img src="(.*?)"/></div></div><div class="cm-body"><div class="name">(.*?)</div><div class="works-num">(.*?)部作品在售</div></div></div></a></li>'
xx=re.compile(wzgz).findall(html)                         #通过正则表达式匹配在网页源代码中提取所需内容
print(xx)

#创建workbook和sheet对象
workbook = xlwt.Workbook()
sheet1 = workbook.add_sheet('sheet1',cell_overwrite_ok=True) #第二参数用于确认同一个cell单元是否可以重设值。
...
workbook.save("豆瓣/" + str(t1)+".xls")                   #保存该excel文件,有同名文件时无法直接覆盖
